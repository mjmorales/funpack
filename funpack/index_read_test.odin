...
test_index_read_schema_mismatch_refused :: proc(t: ^testing.T) {
	...
	expect_refusal(t, mutate_line(t, decl_line, "\"schema_version\":3", "\"schema_version\":1"), .Schema_Mismatch)
	project_line := emit_project_record(minimal_project_record(), context.temp_allocator)
	expect_refusal(t, mutate_line(t, project_line, "\"schema_version\":3", "\"schema_version\":999"), .Schema_Mismatch)
	// A line with no stamp at all is under-shaped before it is mismatched.
	expect_refusal(t, mutate_line(t, decl_line, "\"schema_version\":3,", ""), .Missing_Field)
}
...
test_index_read_missing_key_refused :: proc(t: ^testing.T) {
	...
	expect_refusal(
		t,
		"{\"schema_version\":3,\"entrypoints\":[],\"builds\":[],\"tag_registry\":[],\"pipeline_flattened\":[],\"gate_results\":[]}\n",
		.Missing_Field,
	)
	// A nested gate result missing its passed field.
	expect_refusal(
		t,
		"{\"schema_version\":3,\"entrypoints\":[],\"builds\":[],\"tag_registry\":[],\"capabilities\":[],\"pipeline_flattened\":[],\"gate_results\":[{\"gate\":\"Cyclomatic\"}]}\n",
		.Missing_Field,
	)
}
...
test_index_read_unknown_record_shape_refused :: proc(t: ^testing.T) {
	...
	expect_refusal(t, "{\"schema_version\":3}\n", .Unknown_Record_Shape)
	expect_refusal(t, "{\"schema_version\":3,\"name\":\"native\",\"platform\":\"desktop\"}\n", .Unknown_Record_Shape)
	expect_refusal(t, "{\"schema_version\":3,\"qualified_name\":\"Board\",\"gate_results\":[]}\n", .Unknown_Record_Shape)
}
...
test_index_read_malformed_line_refused :: proc(t: ^testing.T) {
	...
	expect_refusal(t, "42\n", .Malformed_Json)
	expect_refusal(t, "{\"schema_version\":3}{\"schema_version\":3}\n", .Malformed_Json)
}
