...
// collect_imported_fn_records collects the §17 cross-module fn records the
// entrypoint module references — the fns it imports by member name from a sibling
// USER module whose AST is in module_asts. For each import whose leading segment
// names a present sibling module, every imported member that resolves to a
// (non-extern) top-level fn in that module is carried as a Function_Record bearing
// the SEAM module's name in its span, so emit_functions appends a self-contained
// record the runtime resolves by bare name. The result is in import-declaration
// order, then the import's brace-group MEMBER order within each import
// (deterministic — both are source-order slices, and the module_asts map is read by
// key, never iterated).
// A nil/empty module_asts (the single-source path) carries nothing — pong/snake/
// hunt/yard emit byte-for-byte as before. An imported member that is not a fn (a
// type, a const) is skipped here: only fn records belong in [functions], and the
// type/const cross-module surface is the typecheck's concern, not the emitter's.
collect_imported_fn_records :: proc(entry_ast: Ast, module_asts: map[string]Ast) -> []Function_Record {
	...
	for import_node in entry_ast.imports {
		...
		for member in members {
			fn, found := find_fn(seam_ast, member)
			if !found || fn.is_extern {
				// A non-fn member (a type, a const) or an extern fn carries no
				// interpretable body — only declared fn bodies are [functions] records.
				continue
			}
...
// when the name is not a top-level fn (it may be a type or a const, which are not
// [functions] records), so the caller skips it.
