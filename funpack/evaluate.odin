...
// interpreter: the world here is the module's startup population — setup()
// EVALUATED to its [Spawn] batch (resolve_setup_values), the SAME evaluation the
// runtime's [setup] bake serializes its version 0 from — so a query body reads
// exactly the rows the runtime's tick 0 would hold, and the two interpreters
// agree on the read by sharing one evaluator. A module with no setup() (or one
// spawning no T) reads the empty table. Rows materialize in SPAWN ORDER, which IS
// stable Id order (the deterministic spawn counter assigns Ids in batch order),
// so fold/first and the nearest-first tiebreak over this list match the runtime's
// Id-ordered View. The result is a List_Value — the same materialized shape a
// View.of fixture and the runtime's View param binding take — so every list
// combinator composes over it unchanged.
