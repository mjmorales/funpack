// The embedded docs-corpus loader. The committed corpus shards
...
// categories, as the exact lowercase JSON values the shards carry. A section's
// `kind` is exactly one of these; the generator never mints a kind outside this
// set, and corpus_kind_valid is the exhaustive membership test a consumer guards
// on.
...
// Corpus_Section is one indexable unit of documentation. The json: tags pin the
// wire key names (snake/lower) so the marshaled bytes match the committed shards
// regardless of the Odin field spelling. `kind` is a plain string (not an Odin
// enum) so the emitted JSON value is exactly the lowercase category token;
// corpus_kind_valid is the closed-set guard. Anchor is stable across regen
// (slug-derived, never positional) so the index can key on it.
...
// Corpus_Source_Record captures the provenance of one extracted source root.
// content_hash is a hex SHA-256 over the root's concatenated section anchors+text
// (hash_corpus_sections), so a content change between regens is detectable at
// single-root granularity.
...
// the audit trail a stale-corpus check reads. Every field is content-derived (no
// timestamp, no machine path), so
...
// The docs_search/docs_get ranker reads per-kind partitions through this accessor.
...
// by kind. Returns a fixed map keyed by the closed kind values; absent kinds read
// zero. Allocated in `allocator`.
