...
// the path, so the bits are identical on every machine. This is the
// normalize path for decimal literals: a Fixed literal not exactly
// representable (0.1) lands on the nearest Fixed, kernel-evaluable, no
// float fallback.
