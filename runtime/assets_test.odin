...
// v17): loading the committed producer-real dungeon artifact decodes its
// [assets 2] section — one content-addressed 64×32 RGBA8 image (8192 pixel bytes)
// and the atlas (v17 keyed by the manifest handle name `dungeon_atlas`) with its 8
// cell rects — through core:encoding/base64, proving the runtime's DEC_TABLE decode
// round-trips funpack's ENC_TABLE encode byte-for-byte (the Odin-first §29 seam,
// both sides on the same core package).
//
...
// the committed producer-real dungeon artifact — the v17 fixture that carries the
// real [assets 2] sprite section this test round-trips.

// test_assets_decode_round_trips loads dungeon.artifact and asserts its
// [assets 2] section decodes exactly: the dungeon atlas image is 64×32 with a
...
test_assets_decode_round_trips :: proc(t: ^testing.T) {
	...
	// One atlas (keyed by the manifest handle name `dungeon_atlas`, v17),
	// referencing the image by its content hash (the
