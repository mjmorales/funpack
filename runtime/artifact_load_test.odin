...
// The version gate is exact-match: the golden's stamp is the current schema
// version, and the loader builds against exactly that. A wrong stamp or
// version is refused, never parsed (§1): a prior version and any future
// version are mismatches the loader refuses before reading any payload.
@(test)
test_load_version_gate :: proc(t: ^testing.T) {
	...
	// A v11 stamp is a version mismatch — the anchor-less tilemap lead-line
	// schema, refused before any payload.
	...
	// A FUTURE version (v19) is equally a mismatch — the gate is exact, not
	// floor-or-ceiling. It stays exactly one above the current ARTIFACT_SCHEMA_VERSION
	// so the rejection fixture tracks the version frontier as it advances.
	future_version := "funpack-artifact 19\n[meta 0]\n"
...
// the CURRENT stamp: a CollisionLayer enum (§5), a `Body` data decl whose `body`
// field carries a §6 composite default nesting a Vec2 record + a CollisionLayer
...
// emitted. The stamp tracks ARTIFACT_SCHEMA_VERSION (now v8 — the krognid
...
V5_ARTIFACT :: "funpack-artifact 18\n" +
	"[meta 2]\n" +
	"project yard\n" +
	"version L5:0.1.0\n" +
	"[enums 1]\n" +
...
test_load_v5_malformed_refused :: proc(t: ^testing.T) {
	...
	unknown_section := "funpack-artifact 18\n[meta 2]\nproject yard\nversion L5:0.1.0\n[gravity 0]\n"
