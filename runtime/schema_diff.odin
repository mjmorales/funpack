...
//
// PROVENANCE — this kernel is a DELIBERATE COPY of funpack/schema_diff.odin,
// NOT a shared import (the fixed.odin discipline). runtime/** and funpack/**
// are separate products (spec §29, §09); the artifact file is the only
// sanctioned coupling, so this runtime must never link compiler internals.
// The file is therefore self-contained — no imports, no compiler AST types;
// its schema model mirrors the artifact's §6 [data] field record (name, TYPE
// spelling, DEFAULT token, migrate FROM/WITH). The two kernels carry a
// bit-identity OBLIGATION to each other: every declaration below the package
// line is byte-for-byte the funpack kernel's, and the per-class unit vectors
// (schema_diff_test.odin) are asserted on BOTH sides — any change must land
// in both files together or the products' migration plans diverge.
