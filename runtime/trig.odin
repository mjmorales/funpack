...
// audit obligation).
//
// PROVENANCE — this kernel is a DELIBERATE COPY of funpack/trig.odin,
...
// bit-identity OBLIGATION to each other (same Q32.32 bits for every
// angle) — the bet pose-driven replay is bit-identical, enforced by the
// cross-check the trig_test.odin suite asserts (the runtime fixed_sin
// bits equal the funpack fixed_sin bits over a pinned cardinal-angle
// set, spec §10.5). Any change here must be mirrored byte-for-byte in
// funpack/trig.odin (and the golden bits re-asserted in both) or the
// products diverge. The only delta from the funpack original is this
// package line and provenance note.
